...
%PLserialization, graph traversal and workload-specific access method 
%PLoptimizations that demonstrate this flexibility.}
...
%PLscheme in Section~\ref{OASYS}, a graph traversal algorithm in 
%PLSection~\ref{TransClos} and use code transformations to derive optimized application code in Section~\ref{PL}.  Benchmarking figures are provided for each 
...
%PLWe demonstrate that, in contrast to these rigid interfaces, \yad's unique 
%PLblurring of the interface between the application and the storage layer 
%PLenables a new class of automated programming language based optimizations.
...
%PLcould be used to automatically derive this optimization, we implemented 
%PL it manually.}  
...
%PL\footnote{Section~\ref{PL} automatically applies this optimization to the straightforward hashtable.  However, this is not reflected in the numbers reported here.} 
...
%PLimplications.  In this section, we focus on leveraging the simplicity
%PLof \yad's API to implement language based whole-program optimizations.
%PLIn particular, we describe how some of the optimizations that we
%PLimplemented in Section~\ref{optimizedHT} can be automatically
%PLimplemented and applied to any code that follows \yad's API, including
%PLthe straightforward implementation of the hashtable and application
%PLcode that directly accesses raw records.  Before explaining the
%PLoptimization in detail, we describe the existing work that our
%PLtechniques build upon.
%PL
%PL
...
%PL
%PL
%PLAlthough it is not the focus of this section, static analysis techniques
%PLthat verify client code's adherence to library APIs would be
%PLparticularly valuable to \yad, since many of the guarantees provided
%PLby its API involve exceptional situations such as hardware failure,
%PLand code that misuses these APIs can cause data corruption bugs that
%PLonly surface in strange situations.  
%PL
%PLQuite a bit of prior work exists
%PLin the area of API usage checking, including SLAM,~\cite{slam} which is an
%PLiterative tool that uses model checking and reachability analysis to
%PLensure that all possible paths a piece of software may take conform to
%PLcomplex APIs.  In this work, we make use of BLAST,~\cite{blast} a system that is
%PLsimilar to SLAM, but caches intermediate results in order to
%PLfacilitate regression testing.
%PL
%PLBecause \yad provides a wide range of functionality, it is important
%PLthat its API be kept simple if the library is to be easy to use.
%PLHowever, we have found that this makes it difficult to enable
%PLoptimizations that require coordination between multiple layers of the
%PLstorage system.  Calls from high level code to lower level APIs
%PLtend to cause work to be repeated, since the lower level calls must
%PLbe able to handle nearly arbitrary sequences of requests from higher
%PLlevel code.  This prevents them from performing optimizations that are
%PLmade possible by the high level code's structure or semantics.
%PLSimilarly, by definition, high-level code shouldn't be bothered with
%PLthe low-level details of the storage system's implementation.
%PL
%PLMemoization techniques are a common set of design patterns designed to
%PLdeal with this issue.  Essentially, general purpose, deterministic
%PLcode (say a data structure accessor) caches the results of some or all
%PLof the calls made to it, and then checks its cache before recomputing
%PLthe results.  Due to locality of application requests, this
%PLoptimization technique can be surprisingly effective, even in cases
%PLwhere only one result is cached.
%PL
%PLPerformance profiling of \yad reveals that a large fraction of the time it
%PLtakes to handle read-only requests is spent servicing cache hits in
%PLthe page file, despite attempts to optimize this portion of the
%PLlibrary.  The structure of existing code written using \yad's APIs
%PLsuggests that while repeated requests have good temporal locality,
%PLthey also have very good locality with respect to their position on
%PLthe stack.  This suggests that memoization will be much more effective
%PLif applied at call sites instead of within the implementations of the
%PLmemoized functions.
%PL
%PLRather than cluttering \yad's API with calls to support this directly
%PLand encouraging application developers to produce convoluted client code, 
%PLwe derive these optimizations automatically using a source code 
%PLtransformation implemented in Cil.~\cite{cil}  We then use 
%PLBlast to refine and verify the
%PLsoundness of the optimizations.  Blast is run in an iterative process
%PLthat was inspired by Delta Debugging,~\cite{deltaDebugging} and searches for a minimum set
%PLof dynamic checks that result in a verifiably sound program
%PLtransformation.  Build-interceptor~\cite{buildInterceptor} is used to capture preprocessed
%PLversions of software that uses \yad.
%PL
%PLFortunately, these tools are flexible enough to also support a number
%PLof other optimizations and are appropriate for reuse as we develop
%PLtools to verify that client code obeys constraints that are necessary
%PLfor program correctness.  We leave these additional optimizations and
%PLverification tasks to future work.
%PL
%PL%- memoization
...
%PLoptimizer.  Results of calls to \yad's API are automatically cached in
%PLthe stack frame of the caller, and accessed using dynamic checks.
%PLThese dynamic checks are then optimistically removed in an iterative
%PLprocess that verifies the soundness of the program transformation.}
%PL\end{figure}
%PL\subsection{The REGRES Data Store Optimizer}
%PL
%PLOptimization of \yad occurs in in a number of separate phases. (Figure~\ref{fig:regres})  First, the source code is
%PLanalyzed, functions are transformed so that they take additional
%PLarguments, and a set of sound optimizations are introduced.  The
%PLsoundness of the optimizations in this step relies upon the
%PLintroduction of dynamic checks.  At the end of this step, we have produced an
%PLoptimized, compilable version of the original program.
%PL
%PLHowever, at this point the optimized program contains many unnecessary dynamic checks.
%PL  This is problematic for two
%PLreasons.  First, it makes it difficult to tell if the optimization
%PLworked as expected without relying on profiling or code coverage tools
%PLto determine how often the dynamic checks succeed at runtime.  This
%PLmeans it is difficult to obtain feedback regarding the
%PLquality of the code produced by the optimizer.  In turn, this makes it
%PLdifficult for programmers to reason about the implications of changes
%PLto client code, which makes further optimization difficult.  
%PL
%PLSecond,
%PLthe dynamic checks have some cost at runtime, and we would like to
%PLminimize this cost.  In the case of \yad, this seems to be negligible.
%PLHowever, we would like to apply these optimization techniques to other
%PLapplications in the future, so it is important that we minimize this
%PLsource of overhead.  Therefore, \yad's optimizer iteratively refines 
%PLthis version of the code, attempting to locate and remove unnecessary checks.
%PL
%PL\subsection{Cil and Dynamic Checks}
%PL
%PLThe first step of the optimization is based upon the observation that the return value of a
%PLcall to \pin is completely determined by \pin's second argument, a {\em
%PLpageid}.  Furthermore, \yad's calling conventions dictate that
%PLpageid's are passed into functions either as a part of a {\em
%PLrecordid} struct, or as an integer named ``page,'' so there is a
%PLsimple programmatic way to generate a mapping between arguments to
%PLexisting calls and automatically generated Page pointers.
%PL
%PLMost functions that take a pageid as an argument either directly or
%PLindirectly call \pin on the corresponding page.  Therefore, we add an
%PLargument, {\em\PP} to any functions that are passed a pageid but are
%PLnot passed a Page pointer.  At runtime, this argument effectively
%PLprovides access to one memoized result per call site of the
%PLfunction.
%PL
%PLIn the simplest version of the transformation, when the function calls
%PL\pin on the page, it first checks to see if \PP points to the
%PLappropriate page.\footnote{This is done by reading the ``id'' field of
%PLthe Page struct referenced by the pointer.  We arrange for these
%PLpointers to either point to a valid Page struct, or to be NULL.}  If
%PLso, it uses \PP instead of calling \pin.  Otherwise, it proceeds as
%PLnormal.  If it skips the call to \pin, it remembers that it should
%PLalso skip the corresponding call to \unpin.  For concreteness, this would transform this code: 
%PL\begin{verbatim} 
%PL
%PLfoo(int xid, int page) {
%PL   Page * p = loadPage(xid, page);
%PL   ...
%PL}
%PL\end{verbatim}
%PLinto code similar to this:
%PL\begin{verbatim}
%PLfoo_p(int xid, int page, Page * _p) { 
%PL   Page * p = 0;
%PL   int _unpin_p = 0;
%PL   if(!p || p->id != page) { 
%PL      if(_unpin_p) { 
...
%PL}
%PL\end{verbatim}
%PLNote that foo's name has changed.  We will refer to this version of
%PLfoo as the \fP version in the following sections.  Also, note the
%PLredundant checks that do not make use of the fact that p is
%PLinitialized to NULL.  Such checks are necessary to ensure the
%PLsoundness of some of the optimizations described below.  The call to
%PL\checkpage does nothing at runtime;  Blast will 
%PLstatically verify that its arguments are always consistent, verifying 
%PLthat p points to the correct page wherever the original program 
%PLwould have called \pin.
%PL
%PLOf course, we cannot expect the program to behave correctly unless we
%PLupdate the call sites of the transformed functions.  When updating
%PLcall sites, we distinguish between cases where the caller already has
%PLa page pointer handy, and cases where the caller has no page pointer
%PLin its stack frame.  In the case where a page pointer already exists,
%PLwe simply pass the pointer to the child function.
%PL
%PLIf the caller has no page pointer on its stack then the situation is more
%PLcomplicated.  We could pass the address of a page pointer into the child
%PLfunction, allowing it to overwrite the caller's page pointer with a
%PLmore ``interesting'' one.  However, it is unclear how such an
%PL``interesting'' pointer would be defined, especially since this
%PLpointer would be shared by multiple child functions.  Also, we would need
%PLto create a second clone of each function so that a pointer to a
%PLpointer could be passed in.  
%PL
%PLInstead, we opt for a simpler solution, and fall back on the heuristic
%PLthat the pageid passed into the child is probably a reasonable choice.
%PLWe simply add a Page pointer to the caller's stack frame, and then
%PLcall \pin to initialize it to the pageid that is being passed into the
%PLchild function.  In order to handle loops, we check that we are
%PLpassing in the appropriate Page pointer at each call to a child
%PLfunction, and call \pin if necessary.  The
%PLtransformation also arranges for \unpin to be called as appropriate.
%PL
%PLIn results not presented here, we found that this simple
%PLtransformation performed very well on the ``raw read'' benchmark
%PLpresented in Section~\ref{codePerformance} but that it was not able to
%PLsignificantly improve performance for the hash table workload.  It
%PLturns out that this transformation is not very effective on functions
%PLthat take a recordid as an argument, but pass a different recordid
%PLinto child functions, since they pass \PP into their children
%PLfunctions, but \PP is probably associated with with the wrong pageid.
%PLTherefore, each child must call \pin on the appropriate page. 
%PL
%PLUnfortunately, this scenario is relatively common, especially in
%PLimplementations of transactional structures that involve levels of
%PLindirection, such as trees or hash tables.  A simple solution to this
%PLproblem is to add a new Page pointer to functions that take \PP as an
%PLargument, and pass a pageid to child functions.  Then \pin is called
%PLto preload the page, just as it would have been if the \PP argument
%PLdid not exist, except that \PP is reused when appropriate.
%PL
%PLOne final transformation is applied in anticipation of the removal of
%PLdynamic checks described in the next section.  A copy is made of each
%PLfunction that was modified to take a \PP argument.  We call the
%PLoriginal modified version the ``\fP'' version of the function, and the
%PLnew copy the ``\fQ'' version.  The \fQ variant will assume that its callers
%PLpass in a value of \PP that matches the pageid that is passed in, while the \fP version makes no such assumption.
%PLAt this point during optimization, the \fP and \fQ variants are
%PLidentical; the only difference is in the semantics of the API they
%PLexpose to their callers.  The next phase of optimization will attempt 
%PLto replace \fP calls to \fQ calls as it removes unnecessary dynamic
%PLchecks.
%PL
%PL\subsection{Instrumenting for Blast}
%PL\label{instrumenting}
%PLBlast is a static analysis tool that checks code for conformance to a
%PLwell defined API.  In order to use it to verify the soundness of our
%PLoptimizations, we express the optimizations' soundness in terms of
%PLsimple invariants, and then encode these invariants in the transformed
%PLsource as calls to API functions.  We then use Blast to check that
%PLthese API calls are safe.  If it is able to verify the safety of the
%PLcalls, then we know that the optimization is sound.  
%PL
%PLIdeally, this would mean that we could simply create a new,
%PLdummy function, \checkpage, and insert a call to \checkpage
%PLafter each dynamic check introduced above.  We could give the
%PLfollowing spec file to Blast:
%PL
%PL\begin{verbatim}
%PLevent { 
...
%PL\end{verbatim}
%PL
%PLThen, we would have Blast verify that each use of \checkpage is safe.
%PLWe could then iteratively remove dynamic checks, and have Blast
%PLre-verify the file, as we describe in Section~\ref{delta}.
%PLUnfortunately, this straightforward approach places too much burden on
%PLBlast, especially since we invoke it as part of a larger iterative
%PLprocess.  Therefore, we had to perform a number of simplifications
%PLbefore proceeding.
%PL
%PL\subsubsection{Pointer Aliasing}
%PL
%PLThe guard in the original spec file dereferences the page pointer
%PLpassed into \checkpage, which means that Blast must consider pointer
%PLaliasing before verifying that \checkpage is called correctly.
%PLFurthermore, the original implementation of \pin is non-trivial, and
%PLultimately allows aliasing between pointers to different pages unless
%PL\unpin is called correctly.  Also, Blast's pointer analysis is
%PLflow-insensitive.  We found that each of these factors was sufficient
%PLto prevent verification from succeeding even on seemingly ``trivial''
%PLfunctions.  
%PL
%PLFortunately, according to \yad's API, the Page structure's id field is
%PLnever written to outside of \pin and \unpin, and will never be written
%PLto if a function has pinned a page.  Because \pin and \unpin are
%PLdesigned to behave correctly regardless of the order
%PLin which they are called,\footnote{\unpin could misbehave if called on
%PLan invalid pointer, but we do not remove dynamic checks that surround
%PL\unpin.} we are willing to assume these invariants hold.  Therefore,
%PLin the simplified source code that we give to Blast, we redefine
%PL\pin as follows:
%PL
%PL\begin{verbatim}
%PLPage * loadPage(int pageid) {
%PL  return pageid;
%PL}
%PL\end{verbatim}
%PLSimilarly, the spec file is changed to this:
%PL\begin{verbatim}
%PLevent { 
%PL  pattern { $1 = loadPage($2); }
%PL  action { $1 = $2; }
...
%PL\end{verbatim}
%PL
%PLNote that these changes completely bypass the page file, and produce invalid Page pointers.
%PLWe are able to perform this transformation because we will never
%PLactually compile this version of the code, and because (given our
%PLassumptions) it does not affect the soundness of Blast's analysis.  We
%PLwill use similar tricks throughout the rest of this section.
%PL
%PL\subsubsection{Call Graph Simplification}
%PL
%PLThe next step in preparing the code for verification by Blast is to
%PLreduce the amount of code that must be considered per optimization task.
%PLWe achieved this by replacing each call to a \fP or \fQ function with
%PLstatements that non-deterministically assign a value to the variables
%PLthat could be affected by the original function call.  In the case of
%PLcalls to \fQ variants, we also add a \checkpage call to make sure that
%PLthe function is properly invoked.\footnote{Initially, no calls to \fQ
%PLvariants exist.  Instead, they are added as we iteratively run Blast.}
%PL
...
%PL\includegraphics[%
%PL   width=1\columnwidth]{../optimizer/hashTableReadGraph.pdf}
%PL%\vspace{-30pt}
%PL\caption{\sf\label{fig:read}The optimizer significantly speeds up
%PLread only workloads.  As long as the data being accessed fits in page
%PLcache, these workloads are CPU bound.  Therefore, although we report
%PLCPU time here, these numbers are indicative of overall
%PLperformance. Figure~\ref{fig:write} explains why CPU time (and not
%PLwall-clock time) is reported. }
%PL
%PL\end{figure*}
%PL
%PL
%PLWe found that Blast was usually able to process verification requests
%PLover the simplified call-graph, but certain requests took tens of
%PLminutes to complete or simply failed.  Upon inspecting the translated code, we
%PLnoticed that the following call pattern was extremely common:
%PL
%PL\begin{verbatim}
%PLif(!p) { 
%PL  ...
...
%PL  p = loadPage(id);
%PL}
%PLsome_p_variant(p, id);
%PL\end{verbatim}
%PL
%PLBlocks such as this are created when the optimizer decides to
%PLspeculatively load a page before passing it into a child function.
%PLThey have two interesting properties.  First, all possible evaluations
%PLof the large if statement ensure that p->id == id, implying that the
%PL\fP variant at the bottom of the block can be automatically replaced
%PLwith a call to a \fQ variant.  Therefore, we can replace all such calls
%PLbefore passing the program to the iterative Blast evaluator, reducing
%PLthe number of sites that we consider when replacing \fP variants with
%PL\fQ variants.
%PL
%PLSecond, since the block is the result of the optimizer's decision to
%PLspeculatively load a page for a child function, it is probably not
%PLredundant.\footnote{There are a number of scenarios where this is not
%PLthe case, particularly within the bodies of \fQ variants of functions.
%PLWe leave the removal of such dynamic checks to future work.}
%PLTherefore, we collapse the conditional into into a single call to
%PL\pin, simplifying the control flow graph of the optimized code
%PLsignificantly.  
%PL
%PLThis simplification, when combined with the ones described above,
%PLreduces the vast majority of Blast runs to under fifteen seconds, with
%PLa small number of calls taking up to one minute.  Due to the call graph
%PLsimplification described above, we are able to optimize function calls
%PLindependently.  This allows the iterative algorithm described below to
%PLbe feasible, reducing the time to optimize the entire library using
%PLa naive, linear search to less than an hour.
%PL
%PL\subsection{Removing Dynamic Checks}
%PL\label{delta}
%PLThe goal of this phase in the transformation is to remove a subset of
%PLthe dynamic checks added that maximizes some metric.  One possible
%PLmetric would be execution time.  In this metric, each dynamic check
%PLwould need to be assigned some weight proportional to the number of
%PLtimes it is checked during an execution of the program.  This is
%PLnontrivial because the number of times a dynamic check will be
%PLchecked is dependent on the input of the program, and may vary from run
%PLto run. 
%PL
%PLA second metric is merely the number of dynamic checks.  This is
%PLequivalent to assigning each dynamic check equal weight.  In this
%PLformulation, one would simply want to minimize the set of dynamic checks
%PLthat remain in the program.  This is similar to the goal of a phase in
%PLDelta Debugging \cite{deltaDebugging}, where they are looking for a
%PLminimal set of deltas to transform the passing program to the failing
%PLprogram.  We therefore initially tried to design an analogous
%PLbinary-search-like algorithm:
%PL
%PL\begin{verbatim}
%PLdeltaSearch(P, dcList) {
...
%PL
%PL
%PLIn order to tell whether a given set of dynamic checks can be removed
%PLsoundly, we turn to Blast.  First, we ask Blast to verify that the code
%PLis sound with all the dynamic checks in.  After this, we iteratively
%PLsearch by changing the set of removed checks and running the resulting
%PLcode through Blast.  As an example, consider the following naive search
%PLalgorithm used for the numbers cited in this paper \footnote{With the
%PLassumption that all dynamic checks are independent and that all dynamic
%PLchecks have equal weight, this naive search finds the optimal
%PLsolution.}:
%PL
%PL\begin{verbatim}
%PLnaiveSearch(P, dcList)
%PLfor each dynamic check d in dcList
%PL    remove d from P
%PL    run Blast on P
...
%PL\end{verbatim}
%PL
%PLAn important feature of Blast which allows iterative searches to
%PLbe tractable is its regression mode.  In its regression mode, Blast is able
%PLto cache results from its alias analysis and calls to the underlying
%PLtheorem prover for reuse in consecutive runs.
%PL
%PLIn order to simplify theorem proving for Blast, we treated calls to
%PLsubroutines within the program as black boxes.  This allows Blast to
%PLassume that the subroutine returns a nondeterministic value
%PL(maintaining correctness), while cutting down on the execution time of
%PLBlast by not allowing it to spend time analyzing the possible return
%PLvalues of the call.  This, along with the simplification of the call
%PLgraph allowed us to treat each function declaration independently.  This
%PLled us to the following general algorithm for removing dynamic checks:
%PL
%PL\begin{verbatim}
%PLsearch(P, dcList, searchAlgo)
%PLfor each function declaration fd in P
%PL    // remove checks in fn
%PL    call searchAlgo on fn 
%PL\end{verbatim}
%PL
%PLThis is nice because each function declaration that we encountered has
%PLless than 10 dynamic checks, opening up the possibility for more
%PLasymptotically complex search algorithms.  Also, due to the treatment of
%PLsubroutine calls as a black box, the order in which we examine the
%PLfunction declarations is inconsequential.
%PL
%PLOne subtlety we encountered deals with the declaration for functions in the
%PLclass \fQ.  As mentioned above, the body of a function in \fQ is exactly
%PLthe same as its counterpart in \fP.  However, due to application
%PLsemantics, we know that the function in \fQ will only be called with a
%PLmatching \_p and pageid.  In order for Blast to take advantage of this,
%PLwe had to create wrapper functions for each function $f$ in \fQ.  The
%PLwrapper function has the same signature as $f$, but the body explicitly
%PLsets both the \_p and the pageid variable to the same constant value,
%PLand then passes its modified argument list to $f$.  In our LLADD case
%PLstudy, we found that Blast was able to allow us to remove over 50
%PLdynamic checks from functions in \fQ, while it was not able to verify
%PLthe soundness of the removals of any dynamic checks in \fP.
%PL
%PL\begin{figure*}
%PL\includegraphics[%
%PL   width=1\columnwidth]{../optimizer/rawWriteGraph.pdf}
%PL\includegraphics[%
%PL   width=1\columnwidth]{../optimizer/hashTableWriteGraph.pdf}
%PL%\vspace{-30pt}
%PL\caption{\sf\label{fig:write}CPU times for write-only work loads.
%PLThese workloads invoke the logging subsystem, and therefore spend a
%PLsmaller fraction of their time calling \pin and \unpin.  Furthermore,
%PLthese workloads are disk bound, so wall clock times did not show any
%PLmeasurable improvement in performance.  Therefore, CPU times are reported
%PLhere.  To facilitate comparison between the two workloads, CPU time is
%PLalso presented in Figure~\ref{fig:read}.}
%PL
%PL\end{figure*}
%PL
%PL
...
%PL
%PLIn order to evaluate the performance of the optimized code, we ran
%PLoptimized and unoptimized benchmarking software on identical
%PLworkloads.  The current version of our optimizer performs a
%PLwhole-program analysis, and produces a single C source file from Cil's
%PLinternal representation.  This presents a number of problems for
%PLperformance evaluations.  First, Cil's transformation could change the
%PLperformance characteristics of the code (for the better or worse).
%PLSecond, since gcc is invoked on a single compilation unit, it has
%PLmore opportunities to perform optimizations involving code that
%PLoriginally spanned multiple compilation units.  To negate these
%PLeffects, the unoptimized versions of the benchmarks were run through
%PLCil without applying any \yad specific optimizations.  Both versions
%PLof the code were then compiled using the command ``gcc -O3 input.c
%PL-lpthread -lm -lcheck''.  For pragmatic reasons, the numbers presented
%PLin this section reflect the code with all dynamic checks in
%PLplace.\footnote{As mentioned in Section~\ref{instrumenting}, we were
%PLnot able to run Blast directly on an executable version of the code.
%PLWe have not yet produced the (one to one) mapping between call sites
%PLin the version processed by Blast and the call sites in the executable version.}
%PL
%PLRead only operations do not generate any log entries under \yad, and
%PLlargely consist of calling \pin, \unpin and memcpy(), and using
%PLpointer arithmetic to unmarshall information stored in the page file.
%PLTherefore, we expected \pin and \unpin to account for a significant
%PLfraction of the runtime.  Figure~\ref{fig:read} confirms that this is
%PLthe case.
%PL
%PLThe raw read workload uses low level $(page, slot, size)$ recordid
%PLtriples to perform a linear scan over the page file.  This is
%PLbasically a ``best case'' scenario for the optimizer.  The top level
%PLloop contains recordids that can be coupled to page pointers, and
%PLcurrent recordids are good predictors of pages that will be associated
%PLwith future recordids.  Here we see that the runtime of the optimized
%PLversion is less than half that of the original code.
%PL
%PLThe hash table workload accesses hash table keys in sorted order,
%PLproducing random accesses to the page file.  Intuitively, we should not
%PLexpect the optimization to improve matters at all in this case.
%PLHowever, we see that the optimized version completes in roughly 77\%
%PLof the time the original version takes.  The reason for this
%PLimprovement is that the hash table implementation must access a small
%PLamount of metadata in order to service each read.  The metadata
%PLconsists of a header field, which describes the hash table and an
%PLindex page that describes the layout of the hash table's bucket list.
%PLThe optimizer is able to take advantage of this, improving the
%PLread only hash table workload's performance.
%PL
%PLUnfortunately, while the optimizer also improves the CPU utilization of
%PLwrite-only workloads, these workloads are I/O bound, so the
%PLimprovement in CPU utilization does not cause a significant improvement
%PLin execution time.  However, the benchmark workloads do not contain
%PLany application logic.  Therefore, CPU intensive applications built
%PLon top of \yad may become CPU bound even if they are write-intensive.  Also,
%PLmulti-threaded transactional applications are able to amortize I/O
%PLcosts, so optimizations in this case are not wholly without merit.
%PL
%PLWe can see that the reduction in CPU time in the write-only case is
%PLless than the reduction in the read-only case.  This is probably due
%PLto the CPU overhead of the logging API, and other overhead associated
%PLwith performing logical operations in \yad. 
%PL
%PL
%PL
%PL\subsection{Number of dynamic checks removed by Blast}
%PLThe transformation initially added 776 dynamic checks to our LLADD implementation.  As mentioned earlier, 238 of these checks are explicitly ignored during static checking, leaving us with 538 checks.  Manual inspection allowed us to create a preprocessing phase that removed another 238 dynamic checks as explained in Section~\ref{cfgs}, leaving us with 300 checks.  At this point, a majority of the functions (57) that have any dynamic checks in them have exactly one dynamic check in them.  Running our naive search algorithm on the code removes another 121 checks, 38 of them from the 57 functions with exactly one dynamic check in them, bringing us to a grand total of 179 checks.
%PL
%PL\begin{figure*}
%PL\includegraphics[%
...
%PLremoving dynamic checks from the optimized code.  These figures include the
%PLremoval of dynamic checks and changes of calls from \fP to \fQ
%PLvariants.  These figures only include changes made during iterative
%PLruns of Blast and only reflect checks that were actually
%PLconsidered for removal.  The ignored changes and checks consist of
%PL238 calls that were changed from \fP to \fQ variants during
%PLpreprocessing and another 238 dynamic checks that were explicitly
%PLignored during static checking.  Also, the system currently does not
%PLconsider dynamic checks that protect \unpin calls.}
%PL\end{figure*}
%PL
%PL\subsection{Generalization to Other Systems}
%PLAlthough we did not implement the optimizations presented in
%PLthis paper with a general purpose optimization framework in mind, we believe that we
%PLhave identified a number of important properties that allow this class
%PLof optimization to be applied in other situations.  Ideally, we would like to encode
%PLthese properties in a higher level language, allowing this class of 
%PLoptimization to be implemented as a simple description of an APIs properties.
%PL
%PLIn particular, optimizations geared toward memoizing a function require
...
%PLbetween a small set of variables that may be passed into the function
%PLand those that may not.
%PL
%PLThe first point is an obvious prerequisite of any memoization
%PLoptimization that relies upon function arguments to retrieve cached
%PLresults.  
%PL
%PLThe second point is a bit more subtle, as it could be
%PLachieved in at least two ways.  In our case, the ``interesting'' part of the
%PLmemoized result is the id field.  Even
%PLthough this field is not immutable over an entire program run, it is
%PLguaranteed to be immutable until \unpin is called.  If the return
%PLvalue truly is immutable over an entire program run, then the
%PLcomplexities caused by \unpin can be avoided.
%PL
%PLFinally, we used calling conventions and naming conventions to
%PLdistinguish between {\em pageid}'s and other types of variables.  The
%PLpageid type does not explicitly exist within \yad's API.  Instead, we
%PLused heuristics to infer its type.  In other circumstances the API
%PLmight provide rich enough type information to avoid inference
%PLentirely, or a type inference system such as Cqual~\cite{cqual} might be more
%PLappropriate.
%PL
%PLIt may be possible to generalize our system so that it can handle all
%PLof these scenarios.  Such a generalized system seems to be sufficient
%PLto support general purpose, high-level definitions of automated
%PLmemoization transformations.
%PL
%PLAlong these lines, we have mentioned a few other potential approaches
%PLtoward storing memoized results throughout this section.  A potential 
%PLapproach that would complement our method well 
%PLmaintains static, per function (or per call-site) page pointers.  The
%PLpage pointers could store memoized results across calls to the
%PLfunction, and this persistent information could be used in addition to to
%PLthe \PP parameter.
%PL
%PLThe difficulty in implementing this scheme arises because it becomes
%PLunclear how and when \unpin should be called, since static pointers
%PLcould persist indefinitely.  In systems that
%PLmemoize idempotent return values, this would not be an issue, since
%PLthese systems would not contain a call that is analogous to \unpin.
%PL
%PLIn \yad's case, it should be possible to ``partially release'' a page by
%PLcalling \unpin, but keeping a copy of the page pointer.  Before making
%PLan expensive call to \pin the function could speculatively latch
%PLwhatever the partially released pointer points
%PLto.\footnote{\yad already guarantees that it points to a valid Page
%PLstruct} If the pointer still points to the desired page, this
%PLscheme would avoid an expensive hash table lookup.
%PL
...
%PL
%PLWe have designed and implemented a C source to source translation that
%PLimplements high-level, application specific optimizations.  Such
%PLsystems are currently uncommon, but we argue that modern programming
%PLpractices greatly increase their importance.  As far as
%PLwe know, source level, whole program optimization techniques have not
%PLbeen applied to transactional storage systems in the past, as these
%PLsystems tend to provide high-level APIs that are focused on program 
%PLcorrectness and library generality rather than performance.
%PL 
%PLWe are unaware of any other
%PLsystems that take our approach toward program optimization and
%PLbelieve that our combination of aggressive optimization techniques
%PLand conservative runtime checks is particularly valuable.  The use of
%PLa general purpose tool to reduce the overhead associated with runtime
%PLchecks can provide valuable feedback to programmers and reduces the 
%PLcomputational overhead associated with these techniques, while making 
%PLit easier to implement a wide variety of application-specific optimizations.
%PLFinally, we believe
%PLthat by moving the implementation of complex optimizations
%PLto compile time, our system improves the maintainability of software
%PLbuilt with \yad without sacrificing performance.
...
%PLControl Performance Modeling: Alternatives and Implications}. TODS
%PL12(4): (1987) 609-654
%PL
