...
int test(void) {
	...
	unsigned int i, xid;
	const unsigned int INSERT_NUM = 10000; /* should be .75*JB_HASHTABLE_SIZE */
	char value[22]; /* should be log(INSERT_NUM)+8 */
	jbHashTable_t *ht; 

	xid = Tbegin();
	ht = jbHtCreate(xid, 4399);
	...
	printf("insert %u: %lds %ldus\n", INSERT_NUM, total.tv_sec, total.tv_usec);
