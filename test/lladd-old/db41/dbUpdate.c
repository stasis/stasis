...
---*/
/*
 * NOTE: this test assumes that the database has num_xactions * num_inserts_per_xaction
 * strings in the database, whose keys are the sequence 1, 2, 3, ...
 */
...
int main(int argc, char **argv) {
	...
	for (i = 1; i <= num_xactions; i++) {
		...
		for (j = 0; j < num_inserts_per_xaction; j++) {
			...
			dbp->get(dbp, xid, &key, &data, DB_RMW);
			free(data.data);

			string = (char *) malloc(sizeof(char)*data.size);
